...
if ($ValidateOnly) {
  [pscustomobject]@{
    ...
    credentialSource = (Initialize-HybridControlPlaneCredential).Source
...
try {
  if (Test-Path -LiteralPath $Paths.SupervisorPidPath -PathType Leaf) {
    $ExistingSupervisor = Get-HybridVerifiedTunnelSupervisorProcess `
      -Profile $Profile `
...
if (-not $Ready) {
  ...
  if (-not $SupervisorProcess.HasExited) {
    Stop-HybridProcessTree -TargetProcessId $SupervisorProcess.Id
  }
