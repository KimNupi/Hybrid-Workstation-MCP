...
$ExecutablePath = Get-HybridTunnelExecutablePath
$StdioRuntime = Get-HybridStdioRuntime
$Paths = Get-HybridTunnelRuntimePaths -Profile $Profile
$MaximumRestarts = 3
...
function Wait-TunnelReady {
  ...
  while ([DateTimeOffset]::Now -lt $Deadline) {
    ...
    try {
      $HealthBaseUrl = Read-HybridHealthBaseUrl -Path $Paths.HealthUrlPath -ProfileId $Profile.Id
...
if ($ValidateOnly) {
  [pscustomobject]@{
    ok = $true
    profileId = $Profile.Id
    supervisorPath = Get-HybridTunnelSupervisorScriptPath
...
$null = Initialize-HybridControlPlaneCredential
...
if (Test-Path -LiteralPath $Paths.SupervisorPidPath -PathType Leaf) {
  $ExistingSupervisor = Get-HybridVerifiedTunnelSupervisorProcess -Profile $Profile -RecordedPidPath $Paths.SupervisorPidPath
...
try {
  ...
  while (Test-RunRequested) {
    $OperationLock = Enter-HybridTunnelOperationLock -ProfileId $Profile.Id
    $TunnelProcess = $null
    try {
      if (-not (Test-RunRequested)) {
        break
      ...
      if (Test-Path -LiteralPath $Paths.TunnelPidPath -PathType Leaf) {
        $ExistingTunnel = Get-HybridVerifiedTunnelProcess `
          -Profile $Profile `
      ...
      Exit-HybridTunnelOperationLock -Lock $OperationLock
    }
    ...
    if (-not $ReadyResult.Ready) {
      if (-not $TunnelProcess.HasExited) {
        Stop-HybridProcessTree -TargetProcessId $TunnelProcess.Id
    ...
    $Decision = Get-HybridTunnelRecoveryDecision `
