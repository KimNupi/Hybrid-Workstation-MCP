...
try {
  if (Test-Path -LiteralPath $Paths.TunnelPidPath -PathType Leaf) {
    $TunnelProcess = Get-HybridVerifiedTunnelProcess `
      -Profile $Profile `
      -RecordedPidPath $Paths.TunnelPidPath `
      -ExecutablePath $ExecutablePath
    if ($TunnelProcess) {
      Stop-HybridProcessTree -TargetProcessId ([int]$TunnelProcess.ProcessId)
...
if (Test-Path -LiteralPath $Paths.SupervisorPidPath -PathType Leaf) {
  $SupervisorProcess = Get-HybridVerifiedTunnelSupervisorProcess `
    -Profile $Profile `
  ...
  if ($SupervisorProcess) {
    ...
    if (Get-Process -Id ([int]$SupervisorProcess.ProcessId) -ErrorAction SilentlyContinue) {
      Stop-HybridProcessTree -TargetProcessId ([int]$SupervisorProcess.ProcessId)
      Start-Sleep -Milliseconds 150
    }
    if (Get-Process -Id ([int]$SupervisorProcess.ProcessId) -ErrorAction SilentlyContinue) {
