...
foreach ($Case in $Cases) {
  $Decision = Get-HybridTunnelRecoveryDecision -FailureTimes $Case.Failures -Now $Now
...
$Expired = Get-HybridTunnelRecoveryDecision -FailureTimes @($Now.AddMinutes(-10)) -Now $Now
